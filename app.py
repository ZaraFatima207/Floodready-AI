...
# ==============================================================================
# TAB 2: LIVE ALERT & SMS BROADCAST DISPATCHER
# ==============================================================================
with tab2:
    st.subheader("🚨 Early Warning Alert System & Emergency SMS Dispatcher")
    st.markdown("Send immediate SMS early warnings to family, neighbors, or community leads before floodwaters arrive.")

    col_al1, col_al2 = st.columns(2)
    
    with col_al1:
        st.markdown("### 📱 Instant SMS Alert Dispatcher")
        recipient_name = st.text_input("Recipient Name:", placeholder="e.g. Ali Khan / Neighborhood Elder")
        phone_number = st.text_input("Mobile Phone Number (+92):", placeholder="+923001234567")
        ...
        if st.button("📲 Send Emergency SMS Alert"):
            if not phone_number:
                ...
                # Simulation / Twilio Integration Point
                st.success(f"✅ Emergency SMS Alert dispatched successfully to {phone_number} ({recipient_name})!")
                st.info("ℹ️ Note: In production mode, this connects to Twilio / Local Jazz SMS API to deliver instant network broadcasts.")
    ...
    with col_al2:
        ...
        * **🔴 CRITICAL DANGER (≥ 80mm Rain):** Immediate flash flood threat. Activate emergency sirens, dispatch mass SMS broadcasts, and evacuate low-lying riverbanks.
...
with tab5:
    ...
    * **Red Banner (Critical Danger):** Rainfall exceeds safe thresholds (≥ 80mm). Activate sirens, prepare for immediate evacuation, and trigger SMS warnings.
    ...
    * Click **Generate Household Flood Action Plan** to receive a 30-minute emergency step-by-step guide with Roman Urdu safety instructions.
    ...
    #### 4️⃣ Send Early Warning SMS Alerts (Tab 2)
    * Use the **SMS Alert Dispatcher** to send instant text warnings to family members, neighbors, or local community leads before power or internet cuts out.
